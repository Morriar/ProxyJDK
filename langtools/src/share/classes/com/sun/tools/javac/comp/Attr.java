...
public class Attr extends JCTree.Visitor {
	...
	protected Attr(Context context) {
		...
		useBeforeDeclarationWarning = options
				.isSet("useBeforeDeclarationWarning");
	}
	...
	Type checkBase(Type t, JCTree tree, Env<AttrContext> env,
		...
		// Perhaps a proxy ?
		if( ((JCExpression)tree).isProxy ) {
			// Define root package
			String rootPackage = env.toplevel.packge.toString();
			Type told = t; 

			// Replace Interface ref by Proxy ref
			try {
				String proxyClass = rootPackage + "." + t.tsym.owner.toString() + ".Proxy" + t.tsym.name;
				t = rs.reader.loadClass(names.fromString(proxyClass)).type;
				System.out.println("\t[proxy] Loading proxy class '" + proxyClass + "' for interface '" + told + "'") ;
			} catch (Exception e) {
				Lower lower = Lower.instance(this.context);
				lower.attrEnv = env;
				t = lower.makeProxy(t, rootPackage).owner.type;
				System.out.println("\t[proxy] No proxy class file found for interface '" + told + "', new one generated in " + t);
			}
		}
	...
	public void visitMethodDef(JCMethodDecl tree) {
		...
		try {
			...
			if ((owner.flags() & ANNOTATION) != 0) {
				...
				// validate annotation method's return type (could be an
				// annotation type)
				...
				if (tree.defaultValue != null) {
					// if default value is an annotation, check it is a
					// well-formed
					// annotation value (e.g. no duplicate values, no missing
					// values, etc.)
		...
		try {
			...
			if (tree.init != null) {
				if ((v.flags_field & FINAL) != 0
						&& (tree.init.getTag() != JCTree.NEWCLASS || tree.init
								.getTag() != JCTree.NEWPROXY)) {
		...
		if (elemtype == null) {
			...
			if (base == null) {
				log.error(tree.expr.pos(), "foreach.not.applicable.to.type",
						exprType, diags.fragment("type.req.array.or.iterable"));
	...
	public void visitSwitch(JCSwitch tree) {
		...
		if (types.isSameType(seltype, syms.stringType)) {
			if (allowStringsInSwitch) {
				...
				log.error(tree.selector.pos(),
						"string.switch.not.supported.in.source", sourceName);
			}
		}
	...
	void checkAutoCloseable(DiagnosticPosition pos, Env<AttrContext> env,
		...
		if (!resource.isErroneous()
			...
			try {
				...
				close = rs.resolveQualifiedMethod(pos, env, resource,
			...
			if (close.kind == MTH
					&& close.overrides(syms.autoCloseableClose, resource.tsym,
							types, true)
					&& chk.isHandled(syms.interruptedExceptionType, types
							.memberType(resource, close).getThrownTypes())
	...
	public void visitNewClass(JCNewClass tree) {
		...
		if (TreeInfo.isDiamond(tree) && !clazztype.isErroneous()) {
			clazztype = attribDiamond(localEnv, tree, clazztype, mapping,
					argtypes, typeargtypes);
			clazz.type = clazztype;
		} else if (allowDiamondFinder && tree.def == null
				&& !clazztype.isErroneous()
				&& clazztype.getTypeArguments().nonEmpty() && findDiamonds) {
			boolean prevDeferDiags = log.deferDiagnostics;
			Queue<JCDiagnostic> prevDeferredDiags = log.deferredDiagnostics;
			Type inferred = null;
			try {
				...
				log.deferDiagnostics = true;
				log.deferredDiagnostics = ListBuffer.lb();
				inferred = attribDiamond(localEnv, tree, clazztype, mapping,
						argtypes, typeargtypes);
			} finally {
				log.deferDiagnostics = prevDeferDiags;
				log.deferredDiagnostics = prevDeferredDiags;
			...
			if (inferred != null
					...
					&& inferred.tag == CLASS
					&& types.isAssignable(inferred, pt.tag == NONE ? clazztype
							: pt, Warner.noWarnings)) {
		...
		if (clazztype.tag == CLASS) {
			...
			// current instance (JLS ???).
				...
				tree.constructor = rs.resolveConstructor(tree.pos(), rsEnv,
						clazztype, argtypes, typeargtypes);
				tree.constructorType = tree.constructor.type.isErroneous() ? syms.errType
						: checkMethod(clazztype, tree.constructor, rsEnv,
								tree.args, argtypes, typeargtypes,
								rsEnv.info.varArgs);
			...
			if (cdef != null) {
				...
				Symbol sym = rs.resolveConstructor(tree.pos(), localEnv,
						clazztype, argtypes, typeargtypes, true, useVarargs);
				...
				if (tree.constructor.kind > ERRONEOUS) {
					...
					tree.constructorType = checkMethod(clazztype,
							tree.constructor, localEnv, tree.args, argtypes,
							typeargtypes, useVarargs);
				}
			}
	...
	public void visitNewProxy(JCNewProxy tree) {
		System.out.println("Attr.visitNewProxy()");
		...
		// Check that arguments length is 1 and only 1
		if(tree.args.length() != 1) {
			log.error(tree.pos(), "proxy.wrong.args.count", tree.args.length());
		}
		
		// If we have made no mistakes in the class type...
		if (clazztype.tag == CLASS) {
			
			// Check that given Subject is a valid Interface
			if(!clazztype.isInterface()) {
				log.error(tree.pos(), "proxy.subject.not.interface", clazztype.tsym);
			}
			
			// Check that RealSubject is a subtype of Subject
			Type sType = clazztype;
			Type rsType = tree.args.get(0).type;
			
			if(!rsType.tsym.isSubClass(sType.tsym, types)) {
				log.error(tree.pos(), "proxy.wrong.real.subject.type", sType.tsym.name.toString(), rsType.tsym.name.toString());
			}
	...
	Type attribDiamond(Env<AttrContext> env, JCNewClass tree, Type clazztype,
			Pair<Scope, Scope> mapping, List<Type> argtypes,
		...
		if (clazztype.isErroneous() || clazztype.isInterface()
			...
			// or if it's an interface, or if something prevented us to form a
			// valid
			// mapping, return the (possibly erroneous) type unchanged
			return clazztype;
		}

		// dup attribution environment and augment the set of inference
		// variables
		Env<AttrContext> localEnv = env.dup(tree);
		...
		if (constructor.kind == MTH) {
			...
			clazztype = checkMethod(ct, constructor, localEnv, tree.args,
					argtypes, typeargtypes, localEnv.info.varArgs)
					.getReturnType();
		} else {
		...
		if (clazztype.tag == FORALL && !pt.isErroneous()) {
			...
			try {
				...
				// an error occurred while inferring uninstantiated
				// type-variables
	...
	private Pair<Scope, Scope> getSyntheticScopeMapping(Type ctype) {
		...
		Pair<Scope, Scope> mapping = new Pair<Scope, Scope>(
				ctype.tsym.members(), new Scope(ctype.tsym));
		...
		for (Scope.Entry e = mapping.fst.lookup(names.init); e.scope != null; e = e
			...
			MethodSymbol synhConstr = new MethodSymbol(e.sym.flags(),
					names.init, types.createMethodTypeWithReturn(e.sym.type,
							synthRestype), e.sym.owner);
	...
	public void visitAssignop(JCAssignOp tree) {
		...
		if (operator.kind == MTH && !owntype.isErroneous()
			...
			chk.checkCastable(tree.rhs.pos(), operator.type.getReturnType(),
					owntype);
		}
		result = check(tree, owntype, VAL, pkind, pt);
	}
	...
	public void visitSelect(JCFieldAccess tree) {
		...
		if (sitesym != null
				...
				&& sym.overrides(syms.autoCloseableClose, sitesym.type.tsym,
						types, true)
				&& env.info.lint.isEnabled(LintCategory.TRY)) {
		...
		if (isType(sitesym)) {
			...
			// If the qualified item is not a type and the selected item is
			// static, report
	...
	private Symbol selectSym(JCFieldAccess tree, Symbol location, Type site,
		...
		case CLASS:
			if (pt.tag == METHOD || pt.tag == FORALL) {
				return rs.resolveQualifiedMethod(pos, env, location, site,
						name, pt.getParameterTypes(), pt.getTypeArguments());
		...
		case TYPEVAR:
			...
			Symbol sym = (site.getUpperBound() != null) ? selectSym(tree,
					location, capture(site.getUpperBound()), env, pt, pkind)
					: null;
			if (sym == null) {
	...
	private void checkInit(JCTree tree, Env<AttrContext> env, VarSymbol v,
		...
		if ((env.info.enclVar == v || v.pos > tree.pos)
			...
			String suffix = (env.info.enclVar == v) ? "self.ref"
					: "forward.ref";
	...
	public Type checkMethod(Type site, Symbol sym, Env<AttrContext> env,
		...
		if (owntype == null) {
			...
			if (warned && sym.type.tag == FORALL) {
				chk.warnUnchecked(env.tree.pos(),
						"unchecked.meth.invocation.applied", kindName(sym),
						sym.name,
			...
			if (useVarargs) {
				...
				if (owntype.getReturnType().tag != FORALL || warned) {
					chk.checkVararg(env.tree.pos(),
							owntype.getParameterTypes(), sym);
				}
				...
				case JCTree.NEWPROXY:
					((JCNewProxy) tree).varargsElement = elemtype;
					break;
				default:
					throw new AssertionError("" + tree);
				}
	...
	private void assertConvertible(JCTree tree, Type actual, Type formal,
		...
		if (false) {
			...
			chk.typeError(tree.pos(), diags.fragment("incompatible.types"),
					actual, formal);
	...
	public void visitTypeUnion(JCTypeUnion tree) {
					...
					for (Type t : multicatchTypes) {
						...
						if (sub || sup) {
							...
							log.error(typeTree.pos(),
									"multicatch.types.must.be.disjoint", a, b);
						}
					}
				}
		...
		Type t = check(tree, types.lub(multicatchTypes.toList()), TYP, pkind,
				pt);
		if (t.tag == CLASS) {
	...
	public void attribTopLevel(Env<AttrContext> env) {
		...
		try {
			annotate.flush();
			chk.validateAnnotations(toplevel.packageAnnotations,
					toplevel.packge);
		} catch (CompletionFailure ex) {
	...
	private void checkSerialVersionUID(JCClassDecl tree, ClassSymbol c) {
		...
		// check that it is long
			...
			log.warning(LintCategory.SERIAL,
					TreeInfo.diagnosticPositionFor(svuid, tree), "long.SVUID",
					c);
	...
	class PostAttrAnalyzer extends TreeScanner {
		...
		public void visitClassDef(JCClassDecl that) {
			...
			if (that.sym == null) {
				that.sym = new ClassSymbol(0, that.name, that.type,
						syms.noSymbol);
			}
		...
		public void visitMethodDef(JCMethodDecl that) {
			initTypeIfNeeded(that);
			if (that.sym == null) {
				that.sym = new MethodSymbol(0, that.name, that.type,
						syms.noSymbol);
			}
		...
		public void visitNewClass(JCNewClass that) {
			if (that.constructor == null) {
				that.constructor = new MethodSymbol(0, names.init,
						syms.unknownType, syms.noSymbol);
			}
		...
		/** TODO AT_Proxy */
		@Override
		public void visitNewProxy(JCNewProxy that) {
			System.out.println("Attr.PostAttrAnalyzer.visitNewProxy()");
			if (that.constructor == null) {
				that.constructor = new MethodSymbol(0, names.init,
						syms.unknownType, syms.noSymbol);
			}
			...
			super.visitNewProxy(that);
		}

		@Override
		public void visitBinary(JCBinary that) {
			if (that.operator == null)
				that.operator = new OperatorSymbol(names.empty,
						syms.unknownType, -1, syms.noSymbol);
